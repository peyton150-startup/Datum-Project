...
**Similar representation is not evidence of shared ownership.** Merge two concepts only when they have the same source of truth, the same lifecycle, and the same reason to change. This is the complement of the rule above, and it is the more tempting error, because merging looks like removing duplication. `Kind.attribute_schema` and comparison configuration both want per-field metadata and both read as nested dicts, and that resemblance nearly bought them one column and one migration. Apply the three tests and they fail all three: one is authored by whoever declares intent and changes when a kind's surface changes, the other is authored by whoever operates reconciliation and changes when someone tunes noise, and neither is the source of truth for the other. Answer the three questions before comparing shapes, because once two structures look alike the argument for merging them writes itself.

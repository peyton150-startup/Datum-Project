...
Django exception is a fact about the driver, not about `intent`, and the second
caller is already foreseen: `discovery`'s write-race handling is being designed
in issue #57. Placed here now so that design is written against one encoding
rather than adding a second one -- but **only `intent` calls this today**, and a
constant nothing calls is a claim nobody checks, so `UNIQUE_VIOLATION` is not
here. It belongs to whatever #57 decides, if #57 decides it needs one.
"""
...
def sqlstate_of(exc: BaseException) -> str | None:
    ...
    **The cause is nonetheless not the only place looked, and the first code
    ...
    Stated because the previous encoding tested both against its code set and
    this one cannot, and a reader owed that difference should not have to derive
    it. The direction is the safe one: `intent`'s caller would raise its

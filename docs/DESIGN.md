...
**What "counts the loser as a skipped write" means in the code, since the phrase names no counter.** Implemented 2026-08-04 in `_lost_a_write_race` (issue #57). There is no third counter: a lost race counts as **written**, because the resource is in the discovered plane and `read == written + errors` must still hold. The load-bearing half is that it is *not* an error — an error forces `PARTIAL`, and `may_infer_absence` refuses to read deletion into a run that is not `SUCCESS`, so counting a race as a rejection would silently suspend absence detection for that tenant on a run where everything was in fact read and recorded.

Two conditions, both required. The **SQLSTATE** must be `23505` or class 40, read off the exception's cause via `locks.sqlstate_of` — the type is not enough, because the two exits from a race arrive as different Django exceptions and an `OperationalError` that is not a race must never be answered as one. And the resource must **actually be present** on a re-read after the rolled-back savepoint. That second condition is the idempotency check named above: without it, a serialization failure whose winner also rolled back would be reported as a successful write of a row nobody stored, and absence semantics would then treat a resource nobody recorded as present.

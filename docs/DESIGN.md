...
- **`ingest` converts its conflict to a domain exception at its own boundary.** `collector` does not, and the row above says why: its write cannot see the conflict.

**"The collector counts the loser as a skipped write" was a claim about code that never existed, and could not.** It stood in this section for months, was implemented in `_lost_a_write_race` on 2026-08-04, and was removed again on 2026-08-05 after issue #61 showed the handler was unreachable. The correction is worth more than the sentence it replaces, because the reasoning failed in two independent places.

**First, there is no exception to classify.** `_upsert` writes through `update_or_create`, which is built on `get_or_create`, which catches the losing `IntegrityError` itself and retries the *same* natural-key lookup. `uq_discovered_natural_key` is the only unique constraint on the table, so if the constraint fired the retry finds the winner's row and returns it. `_stored` sees a successful upsert. A handler watching for `23505` on this path is dead code, and the three tests written for it reached it only by mocking `_upsert` away entirely — so they never executed the call that absorbs the exception they injected.

**Second, "counts as written" is not the same as having written**, and the difference is what absence believes. `mark_absent_after` selects on `last_seen_run`, not on the run counters. A verdict that the record may be counted written, reached without setting that column, leaves `_unseen_by` collecting the very row the run just declared recorded — so the run ends `SUCCESS` and then flags a live resource absent before returning. That is strictly worse than the outcome the handler was written to avoid: counting the loss as an error merely suspends absence for one cycle, with the resource still recorded present.

**What is actually there is a silent last-write-wins clobber.** The loser's `provider_id`, `attributes` and `last_seen_run` overwrite the winner's, with no exception, no log line, and nothing counting it. That is this row's real vulnerability. It is not presently reachable — see the exclusion argument above — and it is unhandled rather than handled, which is the honest state to record.

**The transferable rule: a row's `last_seen_run` is what absence believes, not the counters.** Any path deciding a resource was observed without touching that column is telling two stories, and `mark_absent_after` will believe the column.
...
**Deliberate strictnesses, each a legal YAML idiom refused on purpose.** An implicit empty value (`v:`) is rejected — not because it is a mistake, but because a declared null must be conspicuous when null-versus-absent is load-bearing. `~`, `NULL` and `Null` are *not* nulls; they are ordinary content read by the declared type, because honouring their tags would hand authority back to the resolver. A merge key is refused **anywhere in the document**, because nothing loads the document any more and a composer does not expand merges — leaving it alone would silently change what a merged value means, worst of all under `metadata`, where an author's `name` could vanish and the document be rejected for a field it visibly declares.

**A merge key is whatever YAML's resolver tags as one, not whatever is spelt `<<`.** The rule was first written as "unquoted, and the text is `<<`", which is a hand-rolled restatement of a decision the resolver has already made — and it disagreed with YAML in both directions. `!!str <<` is an author explicitly declaring a string key, which `main` accepts and the spelling test rejected; `!!merge other` is a merge key not spelt `<<`, which the spelling test missed and then reported as the "unknown attribute" the refusal exists to prevent. Asking for the tag subsumes quoting as well, since `"<<"` already resolves to `str`, so the quoting clause is gone rather than kept beside it. Same rule as `unstorable_attribute` and as §13's own subject: **ask the library, do not model it.**

**Stated limit: an envelope key is its source text, never its resolved type.** A key written `~` or `true` arrives as `"~"` or `"true"` where the loader gave `None` and `True`, and `true` and `"true"` collapse to one key here where the loader kept them apart. This cannot change a document that ingests successfully — every key Datum looks up is a plain string no YAML keyword collides with — but it is visible in duplicate detection and error wording, so it is a limit rather than nothing.

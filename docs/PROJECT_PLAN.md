...
**Three decisions taken before implementation, in #80.** The sink is `logging.getLogger("datum.reconcile.audit")` and nothing else — **`AuditLogEntry` is an in-memory description of an auditable event, not a database row**, and the name is the only thing about it that suggests otherwise. Presence is carried by holding the `PlaneValue`s themselves, which means `declared_raw` and `discovered_raw` are renamed, because those names go false the moment they stop holding raw values. Sampling uses a global `DATUM_AUDIT_SAMPLE_RATE` on a run-scoped writer, with **a counter per `(kind_name, field_name)` stream rather than one counter per run**: a single counter would make one field's emitted entries depend on which other fields exist, and would make iteration order observable. Only `sampled_audit` entries advance a stream, the Nth eligible entry is the one emitted, and N must be at least 1.

Two traps already known:

- **`AuditLogEntry._kind_name` is always `"unknown"` today.** Nothing sets it — it is inert scaffolding waiting for 2H, read at seven sites as `field_config.comparison.get("_kind_name", "unknown")`. Do not assume audit entries carry a kind name, and note that the repeated magic-string lookup is a named constant waiting to happen. **This reaches further than it looks: it makes 2G's sampling stream key `("unknown", field_name)` until 2H populates it, so two kinds declaring a same-named field share one counter.** The key is right and self-heals; the partial behaviour is recorded so it is not rediscovered as a bug.

...
**Three decisions taken before implementation, in #80.** The sink is `logging.getLogger("datum.reconcile.audit")` and nothing else — **`AuditLogEntry` is an in-memory description of an auditable event, not a database row**, and the name is the only thing about it that suggests otherwise. Presence is carried by holding the `PlaneValue`s themselves, which means `declared_raw` and `discovered_raw` are renamed, because those names go false the moment they stop holding raw values. Sampling is owned by a reconciliation-run-scoped writer, with **a counter per `(kind_name, field_name)` stream rather than one counter per run**: a single counter would make one field's emitted entries depend on which other fields exist, and would make iteration order observable.

`DATUM_AUDIT_SAMPLE_RATE` is a global integer, and the rule is exactly:

- **N < 1 — rejected when the writer is constructed.** Not clamped, not sanitised. A bad operational setting must fail immediately and conspicuously rather than quietly change what gets logged: at 0 the modulo raises, and a negative rate emits nothing forever, which is `sampled_audit` meaning "never" under a name that does not say so. The same argument `schema.py` already makes about `tolerance(inf)` being `ignore` under a name that does not say so.
- **N = 1 — every eligible entry emits.**
- **N > 1 — entries N, 2N, 3N… emit, counted independently per stream.**
- **Only `sampled_audit` entries advance a stream.** A `debug` or `discrepancy` entry must not shift the sequence.

Two traps already known:

- **`AuditLogEntry._kind_name` is always `"unknown"` today.** Nothing sets it — it is inert scaffolding waiting for 2H, read at seven sites as `field_config.comparison.get("_kind_name", "unknown")`. Do not assume audit entries carry a kind name. **This reaches further than it looks: it makes 2G's sampling stream key `("unknown", field_name)` until 2H supplies a real one, so two kinds declaring a same-named field share one counter.** The key stays `(kind_name, field_name)` regardless — that is the stable semantic identity, and inventing a temporary 2G-only key would mean changing it back later.

  **What this row got wrong, and it is the more useful half.** It called the repeated lookup "a named constant waiting to happen", and 2G duly made it one — `_kind_name_of`, seven call sites collapsed into a routine. Consolidating a lookup is not the same as checking that the thing should be looked up there, and the tidier version made the wrong home look permanent. The key is not waiting to be populated; it is waiting to be deleted, and the kind name reaches a comparison through `FieldConfig` instead. See the 2H bullet.

  **So cross-kind sampling isolation is not observable in 2G, and no 2G test may claim to demonstrate it.** Every entry carries `kind_name="unknown"`, so a test asserting per-kind independence would pass against a writer that had no such independence at all — the failure mode this plan names as the project's recurring one, aimed at the one test 2G is most tempted to write. **The acceptance test belongs in 2H, where the kind name becomes real: the same field name in two kinds must keep independent counters.**
- **`AuditLogEntry.declared_raw` is `None` for both absent and null**, with presence carried only by the transformed field. Decided in #80, and not the way this row first guessed: it read as conditional on persistence, and 2G persists nothing yet still had to fix it, because a rendered log line collapses the two exactly as a stored row would. The fix is not presence flags either — it is carrying the `PlaneValue`, which already encodes absent, null, and valued as three states. A parallel boolean would be a second encoding of a distinction that already has one.
...
- **The kind name reaches a comparison as identity, which is what activates 2G's per-kind sampling isolation — and `_kind_name` is deleted rather than populated.** This bullet used to say 2H should populate `comparison["_kind_name"]`. That was wrong, and reinforcing it would have been the instruction's fault rather than the implementer's. `ComparisonSchema` **already holds `self.kind_name`**, and then builds each `FieldConfig` without passing it, so the magic key is a second home for a fact that already has one. 2H passes `kind_name` into `FieldConfig` from the schema that owns it.

  **Deleted, not left unused** — an unused path is an attractive one, and the next person to want a kind name inside a config dict will find it already built. Three things go: `_kind_name_of`, `KIND_NAME_KEY`, and `UNKNOWN_KIND_NAME`, all in `comparison.py`.

  **Be precise about what this buys, because the obvious stronger claim is false.** A required `FieldConfig.kind_name` moves kind identity into its proper typed owner instead of recovering it from comparison policy or substituting `"unknown"`. It does **not** by itself make invalid identity unrepresentable: `str` still admits empty or placeholder-like values. Any stronger validity invariant belongs at `ComparisonSchema`/`FieldConfig` construction and has to be enforced explicitly — and 2H should find where kind identity first becomes trusted and validate *there*, rather than mechanically picking `FieldConfig.__post_init__` because that is where the other checks happen.

  **Do not reserve the literal `"unknown"` on the way past.** It is today's placeholder, not a domain rule, and nothing says a kind may not legitimately be named that. An empty name is a clean invariant if kind names are already required to be non-empty elsewhere; the placeholder is not.

  **The boundary is the production comparison path only.** `AuditLogEntry` stays independently constructible, so a writer unit test supplying an arbitrary kind name tests rendering and emission, not kind-identity validation — `test_audit.py`'s `entry(kind_name=...)` helper is legitimate and should stay, with its docstring saying which of the two it is evidence for. What does become stale is prose describing `"unknown"` as runtime behaviour: `AuditLogWriter`'s class docstring and `test_audit.py`'s module docstring both say entries arrive that way, and after 2H they do not.

  Apply the three ownership tests and the old home fails all of them. Kind identity comes from `ComparisonSchema`; mode, tolerance, and precision come from whoever tunes reconciliation noise. Identity changes with the thing being compared; policy changes when an operator retunes it. Identity says *what* is being compared, policy says *how*. **This is the same error #71 rejected between `Kind.attribute_schema` and comparison configuration, one level down and inside the dict rather than beside it.**

  It carries an acceptance test that could not be written in 2G: the same field name declared in two kinds must keep independent sampling counters. The stream key is unchanged — `(kind_name, field_name)` — and #80's Q3 semantics are unchanged with it. What changes is that both halves come from identity instead of one real value and one magic-config value

- **A note on why this took a phase to see, since the next such case will look the same.** The trap bullet below named a hazard — the key is inert, it always reads `unknown`, sampling streams collapse until it is populated — and the author and both blind reviewers on #82 all spent their attention on that consequence. Nobody asked why kind identity was inside comparison policy. Three tells were available and all read as local noise: an underscore-prefixed key in a public dict, a `mypy --strict` complaint about returning `Any` as `str`, and a reviewer naming the unchecked `Any` explicitly. **When a type error is silenced by asserting the expected type, ask whether the real defect is that the value lives in the wrong representation.** An annotation records what the author expects; it does not give the value an owner or a runtime guarantee. This is not an argument against annotations or narrowing — the barricade asserts types on YAML scalars constantly and is right to, because ADR-008 has it *validate* in the same breath. The smell is the annotation being the only thing that makes the claim true. Here the corrected shape gets its type from the data model instead: `ComparisonSchema.kind_name: str` → `FieldConfig.kind_name: str`, with nothing to assert.

  The general form, recorded in CLAUDE.md's two-stage review rationale: naming a hazard tells everyone where to look and quietly tells them what not to question

- **The audit path gets exercised end to end, for the first time.** 2G built the writer and nothing called it, so both blind reviews on #82 certified the writer in isolation. 2H is where that stops being enough: an acceptance test must drive a reconciliation run through comparison, into an `AuditLogEntry`, through the writer, and out as an emitted `datum.reconcile.audit` record. **Emission, not construction** — asserting an entry was built proves only what 2G already proved.

  **The criterion is not satisfied until #83 is.** Audit visibility currently depends on the runner's logging configuration rather than on Datum's, so a test that supplies its own handler passes while production drops the same record — which is exactly how 2G reached 100% branch coverage with nothing emitted anywhere. State the requirement as a property rather than as a banned tool, because a prohibition on `caplog` is satisfied by adding a handler in the fixture by hand: **the test must be capable of failing for a configuration reason.** If the supported application logging configuration stops enabling that record, the test goes red. `caplog` remains right for the writer's own unit tests; this is about the production-path acceptance test only.

  2H does not have to wait on #83 to be implemented. It has to wait on #83 to be able to claim this
...
**Two resolutions, two named steps, even though both currently answer from the same `Kind` row.** #71 decided that comparison configuration lives separately from the declared attribute schema, with `field_type` derived rather than persisted. So 2H's loader has to resolve two different things: **which attribute schema governs this declaration** (which derives `field_type`), and **which comparison policy applies to this field** (mode, parameters, logging). They have different owners, different lifecycles, and different future identities — the attribute schema is the one that later becomes `(kind, schema revision)`, per #70's provenance requirement.

Keep `_load_comparison_schemas()` as the orchestrator, but make it call two named resolutions rather than owning both rules. One blended lookup is where "today's schema is always the right schema" gets baked into reconciliation, invisibly, because the two questions stop being distinguishable inside one function. 2H does **not** implement provenance; it only avoids foreclosing it.

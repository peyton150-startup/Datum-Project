...
- **A note on why this took a phase to see, since the next such case will look the same.** The trap bullet below named a hazard — the key is inert, it always reads `unknown`, sampling streams collapse until it is populated — and the author and both blind reviewers on #82 all spent their attention on that consequence. Nobody asked why kind identity was inside comparison policy. Three tells were available and all read as local noise: an underscore-prefixed key in a public dict, a `mypy --strict` complaint about returning `Any` as `str`, and a reviewer naming the unchecked `Any` explicitly. **A type error fixed by asserting the type, rather than by giving the value a home that has one, is the finding and not the fix.**

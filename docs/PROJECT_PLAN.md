...
- **`_kind_name` gets populated, which is what activates 2G's per-kind sampling isolation.** It carries an acceptance test that could not be written in 2G: the same field name declared in two kinds must keep independent sampling counters. Until this lands, every audit entry reads `kind_name="unknown"` and that isolation is unobservable — see the 2G trap above for why no earlier test may claim it

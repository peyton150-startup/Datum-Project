...
- **The kind name reaches a comparison as identity, which is what activates 2G's per-kind sampling isolation — and `_kind_name` is deleted rather than populated.** This bullet used to say 2H should populate `comparison["_kind_name"]`. That was wrong, and reinforcing it would have been the instruction's fault rather than the implementer's. `ComparisonSchema` **already holds `self.kind_name`**, and then builds each `FieldConfig` without passing it, so the magic key is a second home for a fact that already has one. 2H passes `kind_name` into `FieldConfig` from the schema that owns it.
  ...
  **Deleted, not left unused** — an unused path is an attractive one, and the next person to want a kind name inside a config dict will find it already built. Three things go: `_kind_name_of`, `KIND_NAME_KEY`, and `UNKNOWN_KIND_NAME`, all in `comparison.py`. **And with them goes the value `"unknown"` itself**, because a required `FieldConfig` field has no default: after 2H there is no such thing as an entry without a kind. That invalidates `AuditLogWriter`'s docstring, `test_audit.py`'s module docstring, and its `entry(kind_name="unknown")` fixture default. Expect that churn rather than discovering it.

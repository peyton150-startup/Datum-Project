...
- **The audit path gets exercised end to end, for the first time.** 2G built the writer and nothing called it, so both blind reviews on #82 certified the writer in isolation. 2H is where that stops being enough: an acceptance test must drive a reconciliation run through comparison, into an `AuditLogEntry`, through the writer, and out as an emitted `datum.reconcile.audit` record. **Emission, not construction** — asserting an entry was built proves only what 2G already proved. Note the interaction with #83: audit visibility currently depends on the runner's logging configuration rather than Datum's, so this test either forces that question or is written in a way that hides it
